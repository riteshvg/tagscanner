...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (isRateLimited(email)) {
      return resp(429, { error: 'Daily limit of ' + DAILY_CAP + ' AI requests reached for this email. Try again tomorrow.' });
    }

    console.log(JSON.stringify({ ts: new Date().toISOString(), email: email, type: type }));
    ...
    if (type === 'explain') {
      ...
      const userMsg     = JSON.stringify({ code: code, metadata: metadata || {} });
