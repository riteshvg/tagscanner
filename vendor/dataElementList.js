...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Helper function to check if an entry already exists in the mappings array
  function entryExists(mappings, newEntry) {
    return mappings.some(
      (entry) =>
        entry.dataElement === newEntry.dataElement &&
  ...
  function createSampleData() {
      ...
      analyticsVariables.eVars[`eVar${i}`] = [
        {
          ...
          path: '_experience.analytics.customDimensions.eVars',
        },
      ];
    }
    ...
    for (let i = 1; i <= 3; i++) {
      analyticsVariables.props[`prop${i}`] = [
        {
          dataElement: 'Sample XDM Data Element',
          path: '_experience.analytics.customDimensions.props',
        },
      ];
    }
  ...
  function processXDMPath(deName, obj, currentPath) {
    ...
    for (const key in obj) {
      ...
      if (key.match(/^eVar\d+$/)) {
        ...
        if (!entryExists(analyticsVariables.eVars[key], newEntry)) {
          analyticsVariables.eVars[key].push(newEntry);
        }
      }
      ...
      if (key.match(/^prop\d+$/)) {
        ...
        // Only add if this entry doesn't already exist
        if (!entryExists(analyticsVariables.props[key], newEntry)) {
          analyticsVariables.props[key].push(newEntry);
        }
      }
      ...
      if (key.match(/^event\d+$/)) {
        ...
        // Only add if this entry doesn't already exist
        if (!entryExists(analyticsVariables.events[key], newEntry)) {
          analyticsVariables.events[key].push(newEntry);
        }
      }
  ...
  function updateUI(selectedVariable, mappings) {
    ...
    if (mappings.length > 0) {
      ...
      mappings.forEach((item) => {
        ...
        // Data Element column
        const cellDataElement = row.insertCell(0);
        cellDataElement.innerHTML = `<span class="badge badge-primary">${item.dataElement}</span>`;

        // Path column
        const cellPath = row.insertCell(1);
        cellPath.innerHTML = `<span class="variable-path">${item.path}</span>`;

        // Rules column
        const cellRules = row.insertCell(2);
        ...
        if (rules.length > 0) {
          ...
          uniqueRules.forEach((ruleName) => {
            ...
            ruleItem.className = 'badge badge-warning rule-item';
            ruleItem.textContent = ruleName;
            ruleList.appendChild(ruleItem);
          });

          cellRules.appendChild(ruleList);
        } else {
          cellRules.innerHTML =
