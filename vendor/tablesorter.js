    ...
    var ts = ($.tablesorter = {
      ...
      setup: function (table, c) {
        ...
        c.$table = $table
          ...
          .addClass(
            ts.css.table + ' ' + c.tableClass + tmp + ' ' + c.namespace.slice(1)
          )
          .attr('role', 'grid');
        ...
        c.textExtraction =
          c.$table.attr('data-text-extraction') || c.textExtraction || 'basic';
        // build headers
        ts.buildHeaders(c);
        ...
        c.$headers = $(
          $.map(c.$table.find(c.selectorHeaders), function (elem, index) {
            ...
            if (
              ...
              $elem.html(
                '<div class="' + ts.css.headerIn + '">' + template + '</div>'
            ...
            elem.column = column;
            tmp = ts.getOrder(
              ts.getData($elem, configHeaders, 'sortInitialOrder') ||
                c.sortInitialOrder
            );
          ...
          if (rows.length) {
            ...
            for (indx = 0; indx < max; indx++) {
              ...
              if (header && header.length) {
                ...
                noParser =
                  ts.getData(header, configHeaders, 'parser') === 'false';
      ...
      getParserById: function (name) {
        ...
        for (indx = 0; indx < len; indx++) {
          if (
            ts.parsers[indx].id.toLowerCase() === name.toString().toLowerCase()
          ) {
      ...
      getElementText: function (c, node, cellIndex) {
        ...
        if (typeof extract === 'string') {
          ...
          if (
            extract === 'basic' &&
            typeof (tmp = $node.attr(c.textAttribute)) !== 'undefined'
          ) {
            return $.trim(tmp);
          }
          ...
          for (rowIndex = 0; rowIndex < totalRows; ++rowIndex) {
            ...
            for (colIndex = 0; colIndex < max; ++colIndex) {
              ...
              if (cell && cacheIndex < c.columns) {
                ...
                if (
                  hasParser &&
                  (parsers[cacheIndex].type || '').toLowerCase() === 'numeric'
                ) {
        ...
        if (ts.isEmptyObject(c.cache)) {
          ...
          for (rowIndex = 0; rowIndex < rows; rowIndex++) {
            ...
            for (cellIndex = 0; cellIndex < len; cellIndex++) {
              ...
              if (
                (c.parsers[cacheIndex].type || '').toLowerCase() === 'numeric'
              ) {
                // update column max value (ignore sign)
      ...
      multisort: function (c) {
        ...
        if (ts.debug(c, 'core')) {
          console.log(
            'Applying sort ' + sortList.toString() + ts.benchmark(sortTime)
          );
        }
      },
      ...
      fixColumnWidth: function (table) {
        ...
        if (c.widthFixed && c.$table.children('colgroup').length === 0) {
          ...
          for (index = 0; index < len; index++) {
            percent =
              parseInt(($tbodies.eq(index).width() / overallWidth) * 1000, 10) /
                10 +
              '%';
          ...
          for (setting in orig) {
            ...
            if (typ === 'undefined') {
              ...
              for (setting2 in orig[setting]) {
                typ =
                  ts.defaults[setting] && typeof ts.defaults[setting][setting2];
    ...
    ts.addParser({
      ...
      format: function (str, table) {
        ...
        return str && typeof num === 'number'
          ...
          : str
          ? $.trim(
              str && table.config.ignoreCase ? str.toLocaleLowerCase() : str
            )
          : str;
      },
      type: 'numeric',
    ...
    ts.addParser({
      ...
      format: function (str) {
        ...
        var temp,
          ...
          date = time
            ? new Date(
                '2000/01/01 ' + time.replace(ts.regex.dateReplace, '$1 $2')
              )
            : time;
